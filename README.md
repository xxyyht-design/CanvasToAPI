...
- 🛡️ **主动守护系统 (Custom)**：内置 `ProxyControlService`，支持自动化健康检查与浏览器会话自动唤醒。
...
#### 🛡️ 守护系统配置 (Custom Extension)

| 变量名                  | 描述                                                                       | 默认值  |
| :---------------------- | :------------------------------------------------------------------------- | :------ |
| `AUTO_LAUNCH_BROWSER`   | 是否在检测不到活跃会话时自动拉起浏览器。                                   | `false` |
| `BROWSER_PATH`          | 浏览器可执行文件路径（可选）。留空则尝试使用系统默认浏览器。               | 无      |
